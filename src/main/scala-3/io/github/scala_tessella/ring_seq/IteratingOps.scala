...
trait IteratingOps extends SlicingOps:
  ...
  extension [A, CC[B] <: SeqOps[B, CC, CC[B]]](ring: CC[A])
    ...
    def slidingO(size: Int, step: Int = 1): Iterator[CC[A]] =
      ring.sliceO(0, step * (ring.size - 1) + size).sliding(size, step)
    ...
    def groupedO(size: Int): Iterator[CC[A]] =
      if ring.isEmpty then Iterator.empty
      else
        val n     = ring.size
        val count = (n + size - 1) / size
        ring.sliceO(0, count * size).grouped(size)
    ...
    def zipWithIndexO(from: IndexO = 0): Iterator[(A, Index)] =
      ...
      if n == 0 then Iterator.empty
        ...
        ring.startAt(from).iterator.zipWithIndex.map((a, i) => (a, (start + i) % n))
